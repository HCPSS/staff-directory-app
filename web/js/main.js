...
$("button#dept-submit").click(function(e){
  e.preventDefault();
  var $path = "";
  var $query = $("#search-field2").val();
  var $render = $('#render-cards');
  $.ajax({
    type: "GET",
    url: $path,
    data: 'given-dept=' + $query,
    dataType: "html",
    success: function(response){
      $render.hide().html(response).fadeIn();
    },
...
$("#dept-switch").click(function(e) {
  e.preventDefault();
  $(".dir-search__name--input").fadeOut("slow", function(){
    var styles = {
      position : "relative",
      left : "0px"
    }
    $(".dir-search__dept--input").css( styles ).fadeIn("slow");
  });
  $('#render-cards').hide();
  $('#result-output').hide();
});

$("#staff-switch").click(function(e) {
  e.preventDefault();
  $(".dir-search__dept--input").fadeOut("slow", function(){
    var styles = {
      position : "absolute",
      left : "-9999px"
    }
    $(".dir-search__name--input").fadeIn("slow");
  });
  $('#render-cards').hide();
  $('#result-output').hide();  
});